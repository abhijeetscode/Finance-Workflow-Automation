...
def tool_map_columns(
    ...
    with dspy.context(lm=lm):
        for sheet_match in sheet_matches:
            ...
            sheet_spec = next(
                (
                    s
                    for s in spec.input_sheets
                    if s.semantic_role == sheet_match.semantic_role
                ),
                None,
            )
            if not sheet_spec:
            ...
            sample = tabulate(
                df.head(5), headers="keys", tablefmt="pipe", showindex=False
            )
    ...
    critical_low = any(
        sm.confidence < 0.50 and sm.critical for sm in sheet_matches
    ) or any(cm.confidence < 0.50 and cm.critical for cm in all_column_matches)
    any_medium = any(
        (0.50 <= sm.confidence < 0.70) and sm.critical for sm in sheet_matches
    ) or any(
...
def tool_validate_output(
    ...
    if spec is None:
        return ObservationResult(
            tool="validate_output", success=False, output="Spec not provided."
        )
    ...
    for sheet_spec in spec.output_sheets:
        ...
        for col_spec in sheet_spec.columns:
            if col_spec.name not in df.columns:
                errors.append(
                    f"Missing column '{col_spec.name}' in sheet '{sheet_spec.name}'"
                )
    ...
    for validation in spec.semantic_validations:
        ...
        try:
            if (
                "hold" in validation.name.lower()
                or "hold" in validation.check_logic.lower()
            ):
                ...
                if "Vendor Payments" in xl.sheet_names and mapping:
                    ...
                    if hold_match and hold_match.actual_sheet_name and input_file:
                        hold_df = pd.read_excel(
                            input_file, sheet_name=hold_match.actual_sheet_name
                        )
                        if len(hold_df.columns) > 0:
                            hold_vendors = set(
                                hold_df.iloc[:, 0].dropna().astype(str).str.lower()
                            )
...
def tool_fix_code(
    ...
    if spec is None:
        return ObservationResult(
            tool="fix_code", success=False, output="Spec not provided."
        )
