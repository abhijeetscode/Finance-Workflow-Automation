...
from schema_adapter.tools import OPENAI_TOOLS, TOOLS, ObservationResult
...
class SchemaResolutionSignature(dspy.Signature):
	...
	source_code: str = dspy.InputField(description="Full source code of the execution agent")
...
class SchemaAdapterAgent:
	...
	def build_golden_schema(self) -> GoldenSchema:
		...
		output = self.resolve_step(
			...
			working_file_sample=self._get_file_sample("./inputs/[Simple] Bill processing.xlsx"),
		)
	...
	def run(self, input_file_path: str, max_steps: int = 10) -> list[dict]:
		...
		for step in range(max_steps):
			...
			if not msg.tool_calls:
				logger.warning("Agent responded without calling a tool: %s", msg.content)
				continue

			for tool_call in msg.tool_calls:
				fn_name = tool_call.function.name
				fn_args = json.loads(tool_call.function.arguments)
				logger.info("Tool call: %s args=%s", fn_name, json.dumps(fn_args, indent=2))
					...
					try:
						...
						if fn_name == "execute_agent":
							if not normalized_path:
								result = "normalize_file must be called before execute_agent"
								logger.error(result)
								messages.append(
									{
										"role": "tool",
						...
						if observation.success:
							logger.info("Observation [%s] OK: %s", fn_name, result[:300])
						else:
							logger.error("Observation [%s] FAILED: %s", fn_name, result[:300])
