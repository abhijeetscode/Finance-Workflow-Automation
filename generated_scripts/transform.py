...
def normalize_date_field(date_val, processing_date):
    ...
    try:
        ...
        if (parsed_date.year < processing_date.year or 
            (parsed_date.year == processing_date.year and 
             parsed_date.month < processing_date.month)):
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Process Bill.com vendor payments from Excel file"
    )
    ...
    try:
        ...
        hold_vendors = set(
            str(v).strip().lower() 
            for v in df_holds_list["Payment Hold List"].dropna()
        )
    ...
    try:
        ...
        unpaid_invoices = set(
            df_unpaid["Invoice Number"].astype(str).str.strip()
        )
        prev_uploaded_invoices = set(
            df_prev_uploaded["Invoice Number"].astype(str).str.strip()
        )
