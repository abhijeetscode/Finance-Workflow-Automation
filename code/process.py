...
def main() -> int:
	...
	parser.add_argument("excel_file", help="Path to Excel file with all required sheets")
	...
	# Find source sheet
	source_sheet = next((s for s in xl_file.sheet_names if s.lower().startswith("report")), None)
	if not source_sheet:
	...
	hold_vendors = [str(v).lower() for v in df_payment_holds["Payment Hold List"].dropna()]
	...
	for idx in df_working.index:
		for col in ["Invoice Date", "Due Date", "Approval Date"]:
			...
			if pd.notna(d) and (
				d.year < current_year or (d.year == current_year and d.month < current_month)
			):
	...
	name_map = dict(zip(df_names["Original Vendor Name"], df_names["Corrected Vendor Name"]))
	name_updates = 0
	...
	df_working.to_csv(main_filename, index=False, float_format="%.15g", encoding="utf-8")
